...
function plotall(generate_signal, X, fvec; order=4, p=2, kwargs...)
    ...
    loss = ModelDistance(LS(na=order), RationalOptimalTransportDistance(domain=Continuous(), p=p))
    losses_closedform = tmap(fvec) do f
        SD.evaluate(loss, X,generate_signal(f))
    end
    plot(fvec, rescale(losses_closedform); subplot=1,  lab=L"W_2", legendfont=font(5), layout=2, yscale=:log10, kwargs...)
    plot!(fvec, rescale(losses_closedform); legendfont=font(5), xscale=get(kwargs, :xscale, :identity), xlabel=get(kwargs, :xlabel, ""), subplot=2,  lab="", legend=false, kwargs...)

    loss = WelchOptimalTransportDistance(distmat=nothing, args=(128,), p=p)
    losses_pwelch = map(fvec) do f # segfaults with threads
        # ls_loss(X,generate_signal(f),order)
        SD.evaluate(loss, X,generate_signal(f))
    end
    ...
    loss = ModelDistance(LS(na=order), EuclideanRootDistance(domain=Continuous(), weight=s1 ∘ residueweight, p=p))
    losses_roots_cont = tmap(fvec) do f
        SD.evaluate(loss, X,generate_signal(f))
    end
    ...
    loss = ModelDistance(LS(na=order), SinkhornRootDistance(domain=Continuous(), β=0.0001, p=p))
    losses_sinkhorn = tmap(fvec) do f
        SD.evaluate(loss, X,generate_signal(f), solver=sinkhorn_log!, tol=1e-9, iters=500_000)
    end
...
X = sin.(2pi*0.1.*t)
plotall(generate_signal,X,fvec; order=2, p=2, xscale=:log10, xlabel="Frequency", legend=true)
...
let fvec = exp10.(LinRange(-1.5, log10(0.45), 100)),
    ...
    plotall(generate_signal,X,fvec, order=6, p=2, xlabel="Cutoff Frequency", xscale=:log10)
end
...
let fvec = exp10.(LinRange(-2.5, log10(0.4), 100)),
    ...
    @time plotall(generate_signal,X,fvec, order=6, p=2, xlabel="Cutoff Frequency", xscale=:log10, fillalpha=0.1)
end
