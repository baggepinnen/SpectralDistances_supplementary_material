...
fig1 = plot()
t = 0.1
dist = ClosedFormSpectralDistance(domain=Continuous(), p=2, interval=(0., exp10(1.6)))
...
for t = LinRange(0, 1, 7)
    ...
    plot!(w, sqrt.(Φ), xscale=:log10, yscale=:log10, line_z = t, lab="", xlabel="", title=L"W_2", ylims=(1e-3, 1e1), colorbar=false, l=(1,), c=:viridis)
end
...
Φ2 = bode(tf(A2), w)[1][:]
for t = LinRange(0, 1, 7)
    plot!(w, (1-t).*Φ1 .+ t.*Φ2, xscale=:log10, yscale=:log10, line_z = t, lab="", xlabel="", title=L"L_2", ylims=(1e-3, 1e1), colorbar=false, l=(1,), c=:viridis)
end
...
for t = LinRange(0, 1, 7)
    Φ = interp(w,t)
    plot!(fig3, w, sqrt.(Φ), xscale=:log10, yscale=:log10, line_z = t, lab="", xlabel="", title=L"RD", ylims=(1e-3, 1e1), colorbar=false, l=(1,), c=:viridis)
end
...
for t = LinRange(0, 1, 7)
    Φ = interp(w,t)
    plot!(w, sqrt.(Φ), xscale=:log10, yscale=:log10, line_z = t, lab="", xlabel="", title=L"WRD", ylims=(1e-3, 1e1), colorbar=false, l=(1,), c=:viridis)
end
...
figpoles = plot()
for t = LinRange(0, 1, 30)
    plot!(figpoles, poleinterp(t), marker_z = t, lab="", xlabel="", title="", markerstrokealpha=0, m=(5,), circle=t==0, c=:viridis, xlims=(-0.05,0), colorbar=false, xlabel="Re", ylabel="Im")
end
